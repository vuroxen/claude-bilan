...
async function sendMessage() {
    ...
    if (!text && !selectedFile) {
        console.log("Matn va fayl bo'sh");
        return;
    }
...
function loadMessages() {
    ...
    if (messageListener) {
        db.ref('messages').off('child_added', messageListener);
    }
    
    // Yangi listener
    messageListener = db.ref('messages').limitToLast(100).on('child_added', snap => {
        const data = snap.val();
        
        // Xabar allaqachon bor yoki yo'qligini tekshirish
...
function renderFile(file) {
    ...
    // Boshqa fayllar
    return `<a href="${file.url}" target="_blank" class="msg-file">📎 ${escapeHtml(file.name)}</a>`;
}
...
function syncPin() {
    db.ref('settings/pin').on('value', snapshot => {
        ...
        if (snapshot.exists() && snapshot.val()) {
            if (pinBar) pinBar.style.display = "block";
